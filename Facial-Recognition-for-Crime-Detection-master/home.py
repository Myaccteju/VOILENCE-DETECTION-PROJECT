...
def register(entries, required, menu_var):
    ...
    if len(no_face) > 0:
        ...
        messagebox.showerror("Registration Error", f"Registration failed!\n\n"
                            f"Following images don't contain a face or the face is too small:\n\n{no_face_st}")
        shutil.rmtree(path, ignore_errors=True)
    else:
        # Storing data in the database
        insertData(entry_data)
        rowId = 1  # You may need to adjust this based on your logic for obtaining the rowId
        if rowId >= 0:
            messagebox.showinfo("Success", "Criminal Registered Successfully.")
            shutil.move(path, os.path.join('face_samples', entry_data["Name"]))
            ...
            if not os.path.isdir("profile_pics"):
                os.mkdir("profile_pics")
            cv2.imwrite(f"profile_pics/criminal_{rowId}.png", img_list[profile_img_num])
